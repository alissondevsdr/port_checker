...
export class ExcelProcessor {
  ...
  static async processSpreadsheet(
    ...
    try {
      ...
      const missingCritical = criticalColumns.filter(
        col => columnMapping[col] === null || columnMapping[col] === undefined
      );
      ...
      const notFoundColumns = template.columns.filter(
        col => columnMapping[col.name] === null || columnMapping[col.name] === undefined
      );
      if (notFoundColumns.length > 0) {
        console.log(
          `⚠️ Colunas não encontradas (serão preenchidas com vazio): ${notFoundColumns.map(c => c.name).join(', ')}`
        );
      }
   ...
   */
  private static getStylesForMode(mode: 'simples' | 'normal') {
    if (mode === 'normal') {
      return {
        columnWidths: {
          'Código': 12.33,
   ...
   */
  private static async generateFormattedExcel(
    ...
    headerRow.eachCell((cell, colNumber) => {
      ...
      const bgColor = (styles.columnColors as any)[columnName] || 'FFF3F3F3';
      const align = (styles.columnAlignments as any)[columnName] === 'left' ? 'left' : 'center';
    ...
    for (const row of rows) {
      ...
      dataRow.eachCell((cell, colNumber) => {
        ...
        if (!column) return;
        const columnName = column.name;
        const bgColor = (styles.dataColors as any)[columnName] || 'FFE8E8E8';
        const align = (styles.columnAlignments as any)[columnName] === 'left' ? 'left' : 'center';
    ...
    template.columns.forEach((col, idx) => {
      ...
      column.width = (styles.columnWidths as any)[col.name] || 12.33;
    });
   ...
   */
  static async generateTemplateExcel(mode: 'simples' | 'normal' = 'simples'): Promise<Buffer> {
    const template = getTemplate(mode);
    const styles = this.getStylesForMode(mode);
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Produtos');
    ...
    headerRow.eachCell((cell, colNumber) => {
      ...
      cell.alignment = { horizontal: align as any, vertical: 'top', wrapText: true };
      cell.border = {
        left: { style: 'thin', color: { argb: 'FFA0A0A0' } },
        right: { style: 'thin', color: { argb: 'FFA0A0A0' } },
        top: { style: 'thin', color: { argb: 'FFA0A0A0' } },
    ...
    // Larguras
    template.columns.forEach((col, idx) => {
      worksheet.getColumn(idx + 1).width = (styles.columnWidths as any)[col.name] || 12.33;
    });
